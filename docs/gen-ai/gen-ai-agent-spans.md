...
This is a caller-owned operation span. `gen_ai.agent.*` attributes on this span
identify the **target agent** that is being invoked, not the agent that owns the span.
The target agent's own execution may be represented by a separate `invoke_agent` span;
that execution span MUST NOT carry `gen_ai.agent.interaction.type`.
...
**[5] `gen_ai.agent.interaction.type`:** When the instrumented framework or protocol explicitly distinguishes the operation as a delegation or handoff to another agent.

**[6] `gen_ai.agent.interaction.type`:** This attribute applies to the caller-owned operation that directs work to
...
**[3] `gen_ai.agent.interaction.type`:** When the instrumented framework or protocol explicitly distinguishes the operation as a delegation or handoff to another agent.
